...
class Todo extends Component {
  ...
  render() {
    const { name, completed, id, toggle, moveUp, moveDown, clearAnimations, onEdit } = this.props;
        ...
        <Row>
          ...
          <Row middle={"md"}>
            <Checkbox
              ...
              onChange={toggleTodo}
            />
            <p onClick={() => this.setState({isEditorOpen: true})} className={`${styles.text} ${completed ? styles.completed : ""}`}>
              {name}
            </p>
          </Row>
        </Row>
