...
function ManageReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case TOGGLE_TODO: {
      ...
      const thisTodoItemIndex = todos.findIndex((item) => item.id === action.id);
      let thisTodoItem = todos[thisTodoItemIndex];
      thisTodoItem.completed = !thisTodoItem.completed;
      todos = todos.filter((item, index) => thisTodoItemIndex !== index);
      todos.unshift(thisTodoItem);
      return {
        ...state,
        todos
      };
